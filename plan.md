...
- 🟡 **쿠폰 발급(직접, Phase 2)** — iOS 이식 완료(모델·서비스·상품/발급 탭 UI, 로그인 게이트, 회원권 패턴 미러).
  백엔드는 **tas PR #156**(`coupon-issue.ts` + 가드: 보관·코드형 차단, CI 그린, Draft) — 머지 후 동작. 계약·리뷰 포인트는 `docs/COUPON_ISSUE_BACKEND.md`(CustomerCoupon 테이블 기존재라 마이그레이션 불필요). ⬜ 코드형 발급·결제 차감(Phase 3)은 후속.
